...
if(dataset_name == "cifar10"):
    ...
    std = [0.2470, 0.2435, 0.2616] 
    weight_average_begin = 49
if(dataset_name == "tiny_imagenet"):
    normalize = tiny_imagenet_normalize
    num_classes = 200
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
    weight_average_begin = 24
...
final_hf.push_to_hub("Holmeister/{dataset_name}_{training_type}_{attack_type}_{weight_average_type}_ccm_{ccm}_ccr_{ccr}_results".format(training_type = training_type,
                        attack_type = attack_type, weight_average_type = weight_average_type, ccm = ccm, ccr = ccr, dataset_name = dataset_name), token = hf_token)
...
if detailed_statistics == "True":
    ...
    detailed_hf.push_to_hub("Holmeister/{dataset_name}_{training_type}_{attack_type}_{weight_average_type}_ccm_{ccm}_ccr_{ccr}_detailed_results".format(training_type = training_type,
                            attack_type = attack_type, weight_average_type = weight_average_type, ccm = ccm, ccr = ccr, dataset_name = dataset_name), token = hf_token)
